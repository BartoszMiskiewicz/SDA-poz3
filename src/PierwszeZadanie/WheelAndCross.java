...
//        Scanner scanner = new Scanner(System.in);
        int i =0;
        while (!checkIfGameEnded(gameBoard, i)){
//        for (int i = 0; i < 9; i++) {// podajemy 9 razy cos
...
//            System.out.println();
        ...
        System.out.println("End fo game");
        displayBoard(gameBoard);
    ...
    private static int getPositionFromUser(char[][] gameBoard) {
        ...
        while (!inputFromUserFlag){
            System.out.println("Inert sign position");
    ...
    public static boolean checkIfGameEnded(char[][] gameBoard, int gameStep){
    ...
    public static boolean checkRows(char[][] gameBoard){
        ...
        while (!result && i < gameBoard.length){
            result = (gameBoard[i][0] == gameBoard[i][0] && gameBoard[i][1] == gameBoard[i][2]);
                i++;
            }
            return result;
    }
...
//    TODO convertet to two methods - convertedPositionTo Row, ConverPositionToColumn
